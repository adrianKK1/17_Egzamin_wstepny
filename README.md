...
- [Temat 17 – Egzamin wstępny](#temat-17--egzamin-wstępny)
- [Wersja i dystrybucja](#wersja-i-dystrybucja)
1. [Cel Projektu](#1-cel-projektu)
2. [Założenia Techniczne](#2-założenia-techniczne)
   - 2.1 [Architektura wieloprocesowa](#21-architektura-wieloprocesowa)
   - 2.2 [Wielowątkowość komisji - Work-Stealing Pattern](#22-wielowątkowość-komisji---work-stealing-pattern)
     - [Architektura komisji](#architektura-komisji)
     - [Wątek recepcji (przewodniczący)](#wątek-recepcji-przewodniczący)
     - [Wątki egzaminatorów - Work-Stealing](#wątki-egzaminatorów---work-stealing)
     - [Synchronizacja wątków przy ewakuacji](#synchronizacja-wątków-przy-ewakuacji)
   - 2.3 [Komunikacja IPC (System V)](#23-komunikacja-ipc-system-v)
     - [Kolejki komunikatów (Message Queues)](#kolejki-komunikatów-message-queues)
     - [Pamięć dzielona (Shared Memory)](#pamięć-dzielona-shared-memory)
     - [Semafory](#semafory)
   - 2.4 [Bezpieczeństwo i obsługa błędów](#24-bezpieczeństwo-i-obsługa-błędów)
3. [Logika Symulacji i Wymagania Funkcjonalne](#3-logika-symulacji-i-wymagania-funkcjonalne)
   - 3.1 [Parametry konfiguracyjne](#31-parametry-konfiguracyjne)
   - 3.2 [Przebieg symulacji](#32-przebieg-symulacji)
     - [Faza 1: Inicjalizacja (Dziekan)](#faza-1-inicjalizacja-dziekan)
     - [Faza 2: Weryfikacja matury](#faza-2-weryfikacja-matury)
     - [Faza 3: Komisja A (część teoretyczna)](#faza-3-komisja-a-część-teoretyczna)
     - [Faza 4: Komisja B (część praktyczna)](#faza-4-komisja-b-część-praktyczna)
     - [Faza 5: Ranking i publikacja (Dziekan)](#faza-5-ranking-i-publikacja-dziekan)
   - 3.3 [Ewakuacja (SIGINT)](#33-ewakuacja-sigint)
4. [Wyniki Symulacji](#4-wyniki-symulacji)
   - 4.1 [Pliki wyjściowe](#41-pliki-wyjściowe)
   - 4.2 [Struktura raportu](#42-struktura-raportu)
   - 4.3 [Kolorowanie terminala](#43-kolorowanie-terminala)
5. [Testy Funkcjonalne](#5-testy-funkcjonalne)
   - [Test 1: Weryfikacja ~2% kandydatów bez matury](#test-1-weryfikacja-2-kandydatów-bez-matury)
   - [Test 2: Weryfikacja ~2% poprawkowiczów](#test-2-weryfikacja-2-poprawkowiczów)
   - [Test 3: Ewakuacja (SIGINT) - brak zakleszczeń](#test-3-ewakuacja-sigint---brak-zakleszczeń)
   - [Test 4: Obciążeniowy - duża liczba kandydatów](#test-4-obciążeniowy---duża-liczba-kandydatów)
   - [Test 5: Synchronizacja - brak race conditions w SHM](#test-5-synchronizacja---brak-race-conditions-w-shm)
6. [Elementy Wyróżniające](#6-elementy-wyróżniające)
   - 6.1 [Work-Stealing Pattern](#61-work-stealing-pattern)
   - 6.2 [Enum stanów + automaty](#62-enum-stanów--automaty)
   - 6.3 [Kolorowanie ANSI terminala](#63-kolorowanie-ansi-terminala)
   - 6.4 [Obsługa EINTR (Ctrl+Z / fg)](#64-obsługa-eintr-ctrlz--fg)
   - 6.5 [Graceful shutdown](#65-graceful-shutdown)
7. [Linki do Kodu Źródłowego](#7-linki-do-kodu-źródłowego)
   ...
   - E. [Synchronizacja procesów (wątków)](#e-synchronizacja-procesów-wątków)
...
8. [Struktura Plików Projektu](#8-struktura-plików-projektu)
9. [Problemy Napotkane i Rozwiązania](#9-problemy-napotkane-i-rozwiązania)
   - [Problem 1: Race condition w licznikach pytań](#problem-1-race-condition-w-licznikach-pytań)
   - [Problem 2: Zakleszczenie przy ewakuacji](#problem-2-zakleszczenie-przy-ewakuacji)
   - [Problem 3: Przekroczenie limitu procesów](#problem-3-przekroczenie-limitu-procesów)
